...
Last edit Apr 17, 2:15 P.M 

Objectives Achieved  : Detection , Tracking, Tracking and Detection together 

Objectives to be achieved : Distance Esimation 
...
Mat drawTextPolygon(Mat img, Mat I, vector<Point> &warpedPoints, int matchIndex)
{
	if(!warpedPoints.empty())
	{
		...
		if(matchIndex != -1)
		{
			...
			for(unsigned int i=10; i<template_names[matchIndex].size(); i++)
			{
